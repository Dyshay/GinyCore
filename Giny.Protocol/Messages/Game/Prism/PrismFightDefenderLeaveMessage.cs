...
{     public class PrismFightDefenderLeaveMessage : NetworkMessage  
    {         public  const ushort Id = 9907;
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (subAreaId < 0)
            {
                throw new System.Exception("Forbidden value (" + subAreaId + ") on element of PrismFightDefenderLeaveMessage.subAreaId.");
            }

            fightId = (short)reader.ReadVarUhShort();
            if (fightId < 0)
            {
                throw new System.Exception("Forbidden value (" + fightId + ") on element of PrismFightDefenderLeaveMessage.fightId.");
            }

            fighterToRemoveId = (long)reader.ReadVarUhLong();
            if (fighterToRemoveId < 0 || fighterToRemoveId > 9.00719925474099E+15)
            {
                throw new System.Exception("Forbidden value (" + fighterToRemoveId + ") on element of PrismFightDefenderLeaveMessage.fighterToRemoveId.");
            }
